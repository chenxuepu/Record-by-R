...
names_coef <- c("IC","OM","TDL","DP","MBA","RDA","REA","CEA","CBA","TA","beta","sigma","size","S.P.500.return")
x <- cbind(1,data[,names_coef])
y1 <- data$LTR.rating.categories %>% unclass()
y1 <- (y1 > 4)
start_coef <- glm.fit(x, y1,family = binomial())
coef_1 <- start_coef$coefficients/start_coef$coefficients[names_coef[1]]
# coefs <- start_coef$coefficients
logit <- function(p) log(p/(1 - p))
q <- length(levels(data$LTR.rating.categories))-1
spacing <- logit((1L:q)/(q + 1L))

gammas <- -coef_1[1L] + spacing - spacing[4]
start <- c(coef_1[-1L], gammas)
# start <- start/start[1L]
y <- data$LTR.rating.categories
x <- data[,names_coef] %>% as.matrix()


frist_1_fit <- polr.fit_3(x = x,y = y,start = start,method = "logistic")



eta <- as.matrix(data[,names(no_c$coefficients)])%*%no_c$coefficients %>% drop()
cumpr <- matrix(plogis(matrix(no_c$zeta, nrow(data), q, byrow=TRUE) - eta),ncol =  q)
Y <- t(apply(cumpr, 1L, function(x) diff(c(0, x, 1))))
dimnames(Y) <- list(rownames(data), no_c$lev)
y <-  factor(max.col(Y), levels=seq_along(no_c$lev), labels=no_c$lev)
