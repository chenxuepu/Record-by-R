...
Task_table <- table(Task_Views$Topics) %>%
  as.data.frame() %>%
  arrange(Freq)
hchart(Task_table,"column",hcaes(x=Var1,y=Freq))
```
...
```{r}
Topics <- Topics[Topics %in% c("base","Bayesian","Optimization","MissingData","WebTechnologies","TimeSeries","Multivariate","Cluster","Distributions","Spatial")]
data_package <- data_package[data_package$Topics %in% Topics,]
Topics 
```
