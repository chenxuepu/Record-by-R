...
# save(list = c("mnist","fashion_mnist"),file = "G:/我的雲端硬碟/meeting/mnist.RData")
load("G:/我的雲端硬碟/meeting/mnist.RData")
# while (!exists("mnist")) {
#   try(mnist <- snedata::download_mnist(),)
# }
```
...
table(iris[-index,5],colnames(label)[apply(label,1,which.max)])
...
set.seed(610811101)
iris_tsne_2 <- iris[-index,-5] %>% 
  stsne(theta = 0,check_duplicates = F) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(iris[-index,ncol(iris)]) %>% 
  `names<-`(c("x","y","group"))
...
print("r = 1.5")
set.seed(610811101)
iris_tsne_3 <- cbind(iris[-index,-5],label) %>% 
...
set.seed(610811101)
iris_tsne_3 <- cbind(iris[-index,-5],label) %>% 
  stsne(theta = 0,check_duplicates = F,col_weights = w) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(iris[-index,ncol(iris)]) %>% 
  `names<-`(c("x","y","group"))
...
```{r}
print("Confusion Matrix")
table(mnist[-index,785],colnames(label)[apply(label,1,which.max)])
...
```{r}
print("Confusion Matrix")
table(mnist$class[-index],colnames(label)[apply(label,1,which.max)])
...
print("r = 1.5")
set.seed(610811101)
mnist_tsne_2 <- mnist[-index,1:784] %>% 
  stsne(theta = 0,check_duplicates = F,r = 1.5) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(mnist$class[-index]) %>% 
...
```{r}
print("Confusion Matrix")
table(fashion_mnist[-index,785],colnames(label)[apply(label,1,which.max)])
...
print("r = 1.5")
set.seed(610811101)
fashion_mnist_tsne_2 <- fashion_mnist[-index,1:784] %>% 
  stsne(theta = 0,check_duplicates = F,r = 1.5) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(fashion_mnist[-index,ncol(fashion_mnist)]) %>% 
...
```{r}
print("Confusion Matrix")
table(fashion_mnist$class[-index],colnames(label)[apply(label,1,which.max)])
...
```{r}
print("Confusion Matrix")
table(swiss_roll$class[-index],colnames(label)[apply(label,1,which.max)])
...
swiss_roll_tsne <- swiss_roll[-index,1:3] %>% 
  Rtsne(theta = 0,check_duplicates = F,pca = F) %$% 
  Y %>% 
  as.data.frame() %>% 
...
print("r = 1.5")
set.seed(610811101)
swiss_roll_tsne_2 <- swiss_roll[-index,1:3] %>% 
  stsne(theta = 0,check_duplicates = F,pca = F,r = 1.5) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(swiss_roll$class[-index]) %>% 
  `names<-`(c("x","y","group"))
...
set.seed(610811101)
swiss_roll_tsne_2 <- swiss_roll[-index,1:3] %>% 
  stsne(theta = 0,check_duplicates = F,pca = F) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(swiss_roll$class[-index]) %>% 
  `names<-`(c("x","y","group"))
...
set.seed(610811101)
swiss_roll_tsne_2 <- swiss_roll[-index,1:3] %>% 
  stsne(theta = 0,check_duplicates = F,pca = F,r = 2.5) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(swiss_roll$class[-index]) %>% 
  `names<-`(c("x","y","group"))
...
swiss_roll_tsne_2 <- cbind(swiss_roll[-index,1:3],label) %>% 
  stsne(theta = 0,check_duplicates = F,pca = F,col_weights = w,r = 1.5) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(swiss_roll$class[-index]) %>% 
  `names<-`(c("x","y","group"))
...
set.seed(610811101)
swiss_roll_tsne_2 <- cbind(swiss_roll[-index,1:3],label) %>% 
  stsne(theta = 0,check_duplicates = F,pca = F,col_weights = w) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(swiss_roll$class[-index]) %>% 
  `names<-`(c("x","y","group"))
...
set.seed(610811101)
swiss_roll_tsne_2 <- cbind(swiss_roll[-index,1:3],label) %>% 
  stsne(theta = 0,check_duplicates = F,pca = F,col_weights = w,r = 2.5) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(swiss_roll$class[-index]) %>% 
  `names<-`(c("x","y","group"))
...
```{r}
print("Confusion Matrix")
table(swiss_roll$class[-index],as.numeric(label>0.5))
...
swiss_roll <- snedata::swiss_roll(min_phi = 0,max_phi = 10*pi)
hchart(swiss_roll,"scatter",hcaes(x = x,y=y,color = color))
swiss_roll$r <- sqrt(swiss_roll$x^2+swiss_roll$y^2)

index <- sample(1:nrow(swiss_roll),0.6*nrow(swiss_roll))

hchart(swiss_roll,"scatter",hcaes(x = x,y=y,color = r))


```

```{r}
xgb_swiss_roll <- xgb.DMatrix(data=as.matrix(swiss_roll[index,1:3]),label=swiss_roll$r[index])
...
```{r}
print("train MSE")
mean((swiss_roll$r[index]-predict(xgb_fit,as.matrix(swiss_roll[index,1:3])))^2)
print("test MSE")
mean((swiss_roll$r[-index]-label)^2)
...
swiss_roll_tsne <- swiss_roll[-index,1:3] %>% 
  ...
  Y %>% 
  as.data.frame() %>% 
  cbind(swiss_roll$r[-index]) %>% 
  `names<-`(c("x","y","group"))

hchart(swiss_roll_tsne,"scatter", hcaes(x=x,y=y,color = group)) %>%
  hc_title(text ="swiss_roll") 
...
hchart(swiss_roll_tsne_2,"scatter", hcaes(x=x,y=y,color = group)) %>%
  hc_title(text ="swiss_roll") 
...
```{r}
print("Confusion Matrix")
table(spirals$class[-index],as.numeric(label>0.5)+1)
...
print("r = 1.5")
set.seed(610811101)
spirals_tsne_2 <- spirals[-index,1:2] %>% 
  stsne(theta = 0,check_duplicates = F,pca = F,r = 1.5) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(spirals$class[-index]) %>% 
