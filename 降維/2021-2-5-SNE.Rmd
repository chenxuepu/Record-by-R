...
set.seed(610811101)
iris_sne <- cbind(iris[-index,-5],label) %>% 
  addColWeight(w) %>%
  sneer(method = "asne",plot_type = "none") %$% 
  coords %>% 
  as.data.frame() %>% 
...
swiss_roll_tsne <- swiss_roll_tsne %>% 
  cbind(swiss_roll$color[-index]) %>% 
  `names<-`(c("x","y","group","color"))

hchart(swiss_roll_tsne,"scatter", hcaes(x=x,y=y,color = color)) %>%
  hc_title(text ="swiss_roll") 
```
...
swiss_roll_tsne_2 <- swiss_roll_tsne_2 %>% 
  cbind(swiss_roll$color[-index]) %>% 
  `names<-`(c("x","y","group","color"))

hchart(swiss_roll_tsne_2,"scatter", hcaes(x=x,y=y,color = color)) %>%
  hc_title(text ="swiss_roll") 
```
...
```{r}
set.seed(610811101)
swiss_roll_sne <- swiss_roll[-index,1:3] %>% 
  sneer(method = "asne",plot_type = "none") %$% 
  coords %>%  
  as.data.frame() %>% 
  cbind(swiss_roll$class[-index]) %>% 
...
hchart(swiss_roll_sne,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="swiss_roll") 

swiss_roll_sne <- swiss_roll_sne %>% 
  cbind(swiss_roll$color[-index]) %>% 
  `names<-`(c("x","y","group","color"))

hchart(swiss_roll_sne,"scatter", hcaes(x=x,y=y,color = color)) %>%
  hc_title(text ="swiss_roll") 
```
...
```{r}
set.seed(610811101)
swiss_roll_sne <- cbind(swiss_roll[-index,1:3],label) %>% 
  addColWeight(w) %>% 
  sneer(method = "asne",plot_type = "none") %$% 
  coords %>%  
  as.data.frame() %>% 
...
set.seed(610811101)
swiss_roll_sne <- swiss_roll[-index,1:3] %>% 
  sneer(method = "ssne",plot_type = "none") %$% 
  coords %>%  
  as.data.frame() %>% 
  cbind(swiss_roll$class[-index]) %>% 
  `names<-`(c("x","y","group"))
...
hchart(spirals_sne,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="spirals") 
```
...
```{r}
set.seed(610811101)
spirals_sne <- cbind(spirals[-index,1:2],label) %>% 
  addColWeight(w) %>% 
  sneer(method = "asne",plot_type = "none") %$% 
  coords %>%  
  as.data.frame() %>% 
