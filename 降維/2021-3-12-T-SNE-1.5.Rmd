...
iris_tsne_2 <- iris[-index,-5] %>% 
  wtsne(theta = 0,check_duplicates = F) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(iris[-index,ncol(iris)]) %>% 
  `names<-`(c("x","y","group"))
...
iris_tsne_3 <- cbind(iris[-index,-5],label) %>% 
  wtsne(theta = 0,check_duplicates = F,col_weights = w) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(iris[-index,ncol(iris)]) %>% 
  `names<-`(c("x","y","group"))
...
swiss_roll_tsne_2 <- swiss_roll[-index,1:3] %>% 
  wtsne(theta = 0,check_duplicates = F,pca = F) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(swiss_roll$class[-index]) %>% 
  `names<-`(c("x","y","group"))
...
swiss_roll_tsne_2 <- cbind(swiss_roll[-index,1:3],label) %>% 
  wtsne(theta = 0,check_duplicates = F,pca = F,col_weights = w) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(swiss_roll$class[-index]) %>% 
  `names<-`(c("x","y","group"))
