---
title: "weight-T-SNE"
author: '610811101'
date: "2021/1/8"
output:
  html_document:
    code_folding: hide
    df_print: paged
    toc: yes
    toc_float:
      collapsed: no
      smooth_scroll: yes
---

```{r include=FALSE}
library(highcharter)
library(tidyverse)
library(tidytext)
library(Rtsne)
library(wtsne)
library(magrittr)
library(xgboost)
```

```{r}
# data
mnist <- snedata::download_mnist()
fashion_mnist <- snedata::download_fashion_mnist()
```

# iris

```{r}
xgb_iris <- xgb.DMatrix(data=as.matrix(iris[,-5]),label=as.numeric(iris[,5])-1)
iris_num_class <- length(levels(iris$Species))
xgb_fit <- xgboost(data = xgb_iris, 
eta = 0.5,
max_depth = 10, 
nround=25, 
subsample = 0.7,
colsample_bytree = 0.7,
objective = "multi:softprob",
num_class= iris_num_class,
verbose = 0
)
label <- predict(xgb_fit,as.matrix(iris[,-5]),reshape=T) %>%
  as.data.frame() %>%
  `colnames<-`(levels(iris$Species))
```

```{r}
w <- c(rep(1,ncol(iris[,-5])),rep(5,ncol(label)))
```

## Rtsne

```{r}

set.seed(610811101)
iris_tsne <- iris[,-5] %>% 
  Rtsne(check_duplicates = F) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(iris[,ncol(iris)]) %>% 
  `names<-`(c("x","y","group"))

hchart(iris_tsne,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="iris") 

```

## wtsne

```{r}
set.seed(610811101)
iris_tsne_2 <- cbind(iris[,-5],label) %>% 
  wtsne(check_duplicates = F,col_weights = w) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(iris[,ncol(iris)]) %>% 
  `names<-`(c("x","y","group"))

hchart(iris_tsne_2,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="iris") 

```

# mnist

```{r}
xgb_mnist <- xgb.DMatrix(data=as.matrix(mnist[,-785]),label=as.numeric(mnist[,785])-1)
mnist_num_class <- length(levels(mnist$Label))
xgb_fit <- xgboost(data = xgb_mnist, 
eta = 0.5,
max_depth = 10, 
nround=25, 
subsample = 0.7,
colsample_bytree = 0.7,
objective = "multi:softprob",
num_class= mnist_num_class,
verbose = 0
)
label <- predict(xgb_fit,as.matrix(mnist[,-785]),reshape=T) %>%
  as.data.frame() %>%
  `colnames<-`(levels(mnist$Label))
```

```{r}
w <- c(rep(1,ncol(mnist[,-785])),rep(700,ncol(label)))
```

## Rtsne

```{r}
set.seed(610811101)
mnist_tsne <- mnist[,-785] %>% 
  Rtsne(check_duplicates = F) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(mnist$Label) %>% 
  `names<-`(c("x","y","group"))

hchart(mnist_tsne,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="mnist") 
```

## wtsne

```{r}
set.seed(610811101)
mnist_tsne_2 <- cbind(mnist[,-785],label) %>% 
  wtsne(check_duplicates = F,col_weights = w) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(mnist$Label) %>% 
  `names<-`(c("x","y","group"))

hchart(mnist_tsne_2,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="mnist") 
```

# mnist 2

```{r}
mnist$class <- as.numeric(mnist$Label) %% 3

xgb_mnist <- xgb.DMatrix(data=as.matrix(mnist[,1:784]),label=mnist$class)
mnist_num_class <- length(unique(mnist$class))
xgb_fit <- xgboost(data = xgb_mnist, 
eta = 0.5,
max_depth = 10, 
nround=25, 
subsample = 0.7,
colsample_bytree = 0.7,
objective = "multi:softprob",
num_class= mnist_num_class,
verbose = 0
)
label <- predict(xgb_fit,as.matrix(mnist[,1:784]),reshape=T) %>%
  as.data.frame() %>%
  `colnames<-`(levels(mnist$class))
```

```{r}
w <- c(rep(1,ncol(mnist[,1:784])),rep(700,ncol(label)))
```

## Rtsne

```{r}
set.seed(610811101)
mnist_tsne <- mnist[,1:784] %>% 
  Rtsne(check_duplicates = F) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(mnist$class) %>% 
  `names<-`(c("x","y","group"))

hchart(mnist_tsne,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="mnist") 
```

## wtsne

```{r}
set.seed(610811101)
mnist_tsne_2 <- cbind(mnist[,1:784],label) %>% 
  wtsne(check_duplicates = F,col_weights = w) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(mnist$class) %>% 
  `names<-`(c("x","y","group"))

hchart(mnist_tsne_2,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="mnist") 
```

# fashion mnist

```{r}
xgb_fashion_mnist <- xgb.DMatrix(data=as.matrix(fashion_mnist[,1:784]),label=as.numeric(fashion_mnist[,785])-1)
fashion_mnist_num_class <- length(levels(fashion_mnist$Label))
xgb_fit <- xgboost(data = xgb_fashion_mnist, 
eta = 0.5,
max_depth = 10, 
nround=25, 
subsample = 0.7,
colsample_bytree = 0.7,
objective = "multi:softprob",
num_class= fashion_mnist_num_class,
verbose = 0
)
label <- predict(xgb_fit,as.matrix(fashion_mnist[,1:784]),reshape=T) %>%
  as.data.frame() %>%
  `colnames<-`(levels(fashion_mnist$Label))
```

```{r}
w <- c(rep(1,ncol(fashion_mnist[,1:784])),rep(700,ncol(label)))
```

## Rtsne

```{r}
set.seed(610811101)
fashion_mnist_tsne <- fashion_mnist[,1:784] %>% 
  Rtsne(check_duplicates = F) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(fashion_mnist[,ncol(fashion_mnist)]) %>% 
  `names<-`(c("x","y","group"))

hchart(fashion_mnist_tsne,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="fashion_mnist") 
```

## wtsne

```{r}
set.seed(610811101)
fashion_mnist_tsne_2 <- cbind(fashion_mnist[,1:784],label) %>% 
  wtsne(check_duplicates = F,col_weights = w) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(fashion_mnist[,ncol(fashion_mnist)]) %>% 
  `names<-`(c("x","y","group"))

hchart(fashion_mnist_tsne_2,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="fashion_mnist") 
```

# fashion mnist 2

```{r}
fashion_mnist$class <- 0
fashion_mnist$class[fashion_mnist$Description %in% c("Coat","Shirt","Pullover","T-shirt/top")] <- "top"
fashion_mnist$class[fashion_mnist$Description %in% c("Dress","Trouser")] <- "Dress/Trouser"
fashion_mnist$class[fashion_mnist$Description %in% c("Ankle boot","Sneaker","Sandal","Bag")] <- "Not clothes"
fashion_mnist$class <- as.factor(fashion_mnist$class)
xgb_fashion_mnist <- xgb.DMatrix(data=as.matrix(fashion_mnist[,1:784]),label=as.numeric(fashion_mnist$class)-1)
fashion_mnist_num_class <- length(levels(fashion_mnist$class))
xgb_fit <- xgboost(data = xgb_fashion_mnist, 
eta = 0.5,
max_depth = 10, 
nround=25, 
subsample = 0.7,
colsample_bytree = 0.7,
objective = "multi:softprob",
num_class= fashion_mnist_num_class,
verbose = 0
)
label <- predict(xgb_fit,as.matrix(fashion_mnist[,1:784]),reshape=T) %>%
  as.data.frame() %>%
  `colnames<-`(levels(fashion_mnist$class))
```

```{r}
w <- c(rep(1,ncol(fashion_mnist[,1:784])),rep(700,ncol(label)))
```

## Rtsne

```{r}
set.seed(610811101)
fashion_mnist_tsne <- fashion_mnist[,1:784] %>% 
  Rtsne(check_duplicates = F) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(fashion_mnist$class) %>% 
  `names<-`(c("x","y","group"))

hchart(fashion_mnist_tsne,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="fashion_mnist") 
```

## wtsne

```{r}
set.seed(610811101)
fashion_mnist_tsne_2 <- cbind(fashion_mnist[,1:784],label) %>% 
  wtsne(check_duplicates = F,col_weights = w) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(fashion_mnist$class) %>% 
  `names<-`(c("x","y","group"))

hchart(fashion_mnist_tsne_2,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="fashion_mnist") 
```

# swiss_roll

```{r}
swiss_roll <- snedata::swiss_roll(min_phi = 0,max_phi = 10*pi)
hchart(swiss_roll,"scatter",hcaes(x = x,y=y,color = color))
swiss_roll$class <- sqrt((swiss_roll$x)^2+(swiss_roll$y)^2) %/% (2*pi)

hchart(swiss_roll,"scatter",hcaes(x = x,y=y,group = class))
swiss_roll$class <- as.factor(swiss_roll$class)

xgb_swiss_roll <- xgb.DMatrix(data=as.matrix(swiss_roll[,1:3]),label=as.numeric(swiss_roll$class)-1)
swiss_roll_num_class <- length(levels(swiss_roll$class))
xgb_fit <- xgboost(data = xgb_swiss_roll, 
eta = 0.5,
max_depth = 10, 
nround=25, 
subsample = 0.7,
colsample_bytree = 0.7,
objective = "multi:softprob",
num_class= swiss_roll_num_class,
verbose = 0
)
label <- predict(xgb_fit,as.matrix(swiss_roll[,1:3]),reshape=T) %>%
  as.data.frame() %>%
  `colnames<-`(levels(swiss_roll$class))
```

```{r}
w <- c(rep(1,ncol(swiss_roll[,1:3])),rep(20,ncol(label)))
```

## Rtsne

```{r}
set.seed(610811101)
swiss_roll_tsne <- swiss_roll[,1:3] %>% 
  Rtsne(check_duplicates = F,pca = F) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(swiss_roll$class) %>% 
  `names<-`(c("x","y","group"))

hchart(swiss_roll_tsne,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="swiss_roll") 
```

## wtsne

```{r}
set.seed(610811101)
swiss_roll_tsne_2 <- cbind(swiss_roll[,1:3],label) %>% 
  wtsne(check_duplicates = F,pca = F,col_weights = w) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(swiss_roll$class) %>% 
  `names<-`(c("x","y","group"))

hchart(swiss_roll_tsne_2,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="swiss_roll") 
```

# swiss_roll 2

```{r}
swiss_roll <- snedata::swiss_roll(min_phi = 0,max_phi = 10*pi)
hchart(swiss_roll,"scatter",hcaes(x = x,y=y,color = color))
swiss_roll$class <- (sqrt((swiss_roll$x)^2+(swiss_roll$y)^2) %/% (2*pi)) %% 2

hchart(swiss_roll,"scatter",hcaes(x = x,y=y,group = class))
swiss_roll$class <- as.factor(swiss_roll$class)

```

```{r}
xgb_swiss_roll <- xgb.DMatrix(data=as.matrix(swiss_roll[,1:3]),label=as.numeric(swiss_roll$class)-1)
swiss_roll_num_class <- length(levels(swiss_roll$class))
xgb_fit <- xgboost(data = xgb_swiss_roll, 
eta = 0.5,
max_depth = 10, 
nround=25, 
subsample = 0.7,
colsample_bytree = 0.7,
objective = "binary:logistic",
verbose = 0
)
label <- predict(xgb_fit,as.matrix(swiss_roll[,1:3])) 
```

```{r}
w <- c(rep(1,ncol(swiss_roll[,1:3])),20)
```

## Rtsne

```{r}
set.seed(610811101)
swiss_roll_tsne <- swiss_roll[,1:3] %>% 
  Rtsne(check_duplicates = F,pca = F) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(swiss_roll$class) %>% 
  `names<-`(c("x","y","group"))

hchart(swiss_roll_tsne,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="swiss_roll") 
```

## wtsne

```{r}
set.seed(610811101)
swiss_roll_tsne_2 <- cbind(swiss_roll[,1:3],label) %>% 
  wtsne(check_duplicates = F,pca = F,col_weights = w) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(swiss_roll$class) %>% 
  `names<-`(c("x","y","group"))

hchart(swiss_roll_tsne_2,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="swiss_roll") 
```

# mlbench.spirals

```{r}
mlbench.spirals <- mlbench::mlbench.spirals(500,cycles = 3,sd = 0.04)
spirals <- cbind(mlbench.spirals$x,mlbench.spirals$classes) %>%
  as.data.frame() %>%
  `names<-`(c("x","y","class"))
hchart(spirals,"scatter",hcaes(x = x,y=y,group = class))
```

```{r}
xgb_spirals <- xgb.DMatrix(data=as.matrix(spirals[,1:2]),label=as.numeric(spirals$class)-1)
spirals_num_class <- length(levels(spirals$class))
xgb_fit <- xgboost(data = xgb_spirals, 
eta = 0.5,
max_depth = 10, 
nround=25, 
subsample = 0.7,
colsample_bytree = 0.7,
objective = "binary:logistic",
verbose = 0
)
label <- predict(xgb_fit,as.matrix(spirals[,1:2])) 
```

```{r}
w <- c(rep(1,ncol(spirals[,1:2])),10)
```

## Rtsne

```{r}
set.seed(610811101)
spirals_tsne <- spirals[,1:2] %>% 
  Rtsne(check_duplicates = F,pca = F) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(spirals$class) %>% 
  `names<-`(c("x","y","group"))

hchart(spirals_tsne,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="spirals") 
```

## wtsne

```{r}
set.seed(610811101)
spirals_tsne_2 <- cbind(spirals[,1:2],label) %>% 
  wtsne(check_duplicates = F,pca = F,col_weights = w) %$% 
  Y %>% 
  as.data.frame() %>% 
  cbind(spirals$class) %>% 
  `names<-`(c("x","y","group"))

hchart(spirals_tsne_2,"scatter", hcaes(x=x,y=y,group = group)) %>%
  hc_title(text ="spirals") 
```
