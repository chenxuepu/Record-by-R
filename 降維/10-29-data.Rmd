...
```{r eval=FALSE, include=FALSE}
hchart(icd_short_mds,"scatter", hcaes(x=x,y=y,group = chapter,label = code ,value = short_desc)) %>%
...
```{r eval=FALSE, include=FALSE}
hchart(icd_long_mds,"scatter", hcaes(x=x,y=y,group = chapter,label = code ,value = long_desc)) %>%
  hc_title(text ="ICD-10") %>%
  hc_chart(zoomType ="xy") %>%
  hc_tooltip(formatter = JS("function () {
                                   return '<b>' + this.point.label + '</b><br/>'+'Description: '+this.point.value;}"))
...
```{r}
hchart(icd_short_tsne,"scatter", hcaes(x=x,y=y,group = chapter,label = code ,value = short_desc)) %>%
  hc_title(text ="ICD-10") %>%
  hc_chart(zoomType ="xy") %>%
  hc_tooltip(formatter = JS("function () {
                                   return '<b>' + this.point.label + '</b><br/>'+'Description: '+this.point.value;}"))
...
hchart(icd_long_tsne,"scatter", hcaes(x=x,y=y,group = chapter,label = code ,value = long_desc)) %>%
  hc_title(text ="ICD-10") %>%
  hc_chart(zoomType ="xy") %>%
  hc_tooltip(formatter = JS("function () {
                                   return '<b>' + this.point.label + '</b><br/>'+'Description: '+this.point.value;}"))
...
```{r}
hchart(icd_short_umap,"scatter", hcaes(x=x,y=y,group = chapter,label = code ,value = short_desc)) %>%
  hc_title(text ="ICD-10") %>%
  hc_chart(zoomType ="xy") %>%
  hc_tooltip(formatter = JS("function () {
                                   return '<b>' + this.point.label + '</b><br/>'+'Description: '+this.point.value;}"))
...
```{r}
finaldata_mds_model <- finaldata %>% 
  dist() %>% 
  cmdscale() %>%
  as.data.frame() %>%
  `names<-`(c("x","y"))

finaldataForlm <- cbind(finaldata_mds_model,finaldata)
lm(x~.-y,data = finaldataForlm) %>% summary()
lm(y~.-x,data = finaldataForlm) %>% summary()
```
...
finaldata_mds <- finaldata_mds_model %>%
...
```{r}
data_in_mds_model <- data_in[,4:15] %>% 
  dist() %>% 
  cmdscale() %>%
  as.data.frame() %>%
  `names<-`(c("x","y"))

data_inForlm <- cbind(data_in_mds_model,data_in[,4:15])
lm(x~.-y,data = data_inForlm) %>% summary()
lm(y~.-x,data = data_inForlm) %>% summary()
```
