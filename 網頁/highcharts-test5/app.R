...
server <- function(input, output) {
    ...
    output$Plot6 <- renderHighchart({
        data$shop[data$ym==input$shop_choice] %>% 
            table() %>% 
            sort(decreasing = T) %>% 
            .[1:10] %>% 
            data.frame() %>%
            hchart("pyramid",hcaes(x = ., y = Freq)) %>%
            hc_title(text =input$shop_choice) 
    })
    output$Plot7 <- renderHighchart({
        data$id[data$ym==input$people_choice] %>% 
            table() %>% 
            sort(decreasing = T) %>% 
            .[1:10] %>% 
            data.frame() %>%
            hchart("pyramid",hcaes(x = ., y = Freq)) %>%
            hc_title(text =input$people_choice)
    })
    x <- reactiveValues()
    x$data <- 0
    ...
    output$test <- renderUI({
        if(x$data==0){
            ...
            tabsetPanel(
                ...
                tabPanel("weekday",highchartOutput("Plot5")),
                tabPanel("shop",
                         selectInput("shop_choice", label = h3("which month"),
                                     choices = ym, 
                                     selected = 1),
                         highchartOutput("Plot6")
                         ),
                tabPanel("people",
                         selectInput("people_choice", label = h3("which month"),
                                     choices = ym, 
                                     selected = 1),
